...
- `container/entrypoint.ts` — Runs inside Docker: reads stdin, builds system prompt from living files + message history, passes mcpServers to SDK `query()`, emits result between markers
...
- `skills/` — Simple skills directory (CLI scripts/API wrappers, mounted read-only into containers)
